...
function registerTmpl(name, template, cache, components) {
  ...
  return function (target) {
    ...
    //创建模板函数
    if (template) {
      target.prototype.template = (template._njTmpl ? template : __WEBPACK_IMPORTED_MODULE_0_nornj___default.a.compileH(template, name)).bind({ _njIcp: __WEBPACK_IMPORTED_MODULE_0_nornj___default.a.isArray(components) ? components : [components] });
    }
