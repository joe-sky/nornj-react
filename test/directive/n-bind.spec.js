...
nj.registerExtension(
  ...
  options => {
    ...
    tagProps.onChange = function (e) {
      $this.setState(
        putStateValue(_value, e.target.value),
        () => _onChange.apply($this, arguments)
      );
    };
  }
);
...
describe('n-stateBind directive', function () {
  ...
  it("responds to value change", () => {
    ...
    const wrap = mount(
      <TestStateBind handleChange={spyHandleChange} />
    );
...
describe('n-bind directive', function () {
  ...
  it("responds to name change", () => {
    ...
    const wrap = mount(
      <TestBind handleChange={spyHandleChange} inputRef={ref} />
    );
